...
// - The remaining features (macro/Fed-rate data, implied volatility,
//   economic-calendar proximity) are NOT recomputed here -- they come
//   from whatever the last hourly run found, since their own underlying
//   data sources only update on their own daily/6-hourly schedules
//   regardless, so there is nothing genuinely "live" to fetch for them
//   on a per-question basis. News sentiment IS refreshed to the latest
//   already-computed reading (see fetchFreshestNewsSentiment below) --
//   free to do since it's just a GitHub file fetch, not a new paid API
//   call, closing a real staleness gap without any new cost or risk.
...
const NEWS_SENTIMENT_HISTORY_RAW_URL =
  "https://raw.githubusercontent.com/iAi2025Joy/gold-predictor/main/news_sentiment_history.json";

// Fetches the FRESHEST already-computed news sentiment reading directly
// -- this file is updated every 6 hours by news_sentiment_fetcher.py and
// is free to fetch from GitHub (no Alpha Vantage API cost). Using this
// instead of whatever sentiment value was baked into the cached HOURLY
// gold prediction closes a real, if modest, staleness gap: the hourly
// prediction only picks up a new sentiment reading on its next run
// (up to ~1 hour late), while this fetches the true latest reading
// directly, every time. Deliberately does NOT make a new live Alpha
// Vantage call itself -- that API's free tier is a shared 25
// requests/day budget across several existing scheduled jobs, and an
// uncapped live call per chat question could exhaust it quickly under
// real traffic. Returns null (not a fallback value) if anything goes
// wrong, so the caller can cleanly fall back to the cached hourly value
// instead.
...
export async function runLiveGoldInference(livePrice, cachedData) {
  ...
  try {
    const [historyResp, artifactsResp, sentimentResult] = await Promise.all([
      ...
      fetchFreshestNewsSentiment(), // free (no API cost), so always attempted -- not just when markets are closed
    ]);
    if (!historyResp.ok || !artifactsResp.ok) return null;
    history = await historyResp.json();
    artifacts = await artifactsResp.json();
    freshestSentiment = sentimentResult; // null if unavailable -- falls back to cached value below
  } catch (err) {
    return null;
  }
  ...
  // Use the genuinely freshest sentiment reading when available (fetched
  // directly above, free, no API cost), falling back to whatever was
  // baked into the cached hourly prediction only if that fetch failed.
  ...
  const rawValues = featureOrder.map((name) => {
    ...
    // specifically are NOT recomputed live) -- except news sentiment,
    // which IS refreshed above since doing so costs nothing.
