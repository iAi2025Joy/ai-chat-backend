      ...
      const messages = [
        {
          ...
          content:
            ...
            "A CUSTOM OUTLINE IS A FLOOR, NOT A CEILING -- a confirmed real bug this fixes: given a detailed, professional, section-by-section outline naming venues like IEEE/ACM/USENIX, a delivered paper matched that outline's own structure well, but came in under the person's own stated abstract word target, and had no standalone ethics section at all -- because the person's own outline happened not to itemize one, even though it's one of GARNET's own standing requirements. When someone gives their own detailed structure/outline, follow it closely, but treat it as the MINIMUM required structure, not the complete list of everything the paper needs -- every other requirement defined elsewhere in this system (a real ethics section wherever it applies, real synthesized literature review, no fabricated results, real relevant references) still applies IN FULL on top of their outline, even for sections they didn't explicitly list. " +
