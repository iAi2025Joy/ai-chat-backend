...
import { synthesizeArabicSpeech, getAvailableVoices } from "./piperTTS.js";
...
// ------------------------------------------------------------------
// PIPER TTS -- real, free, open-source Arabic speech generation,
// running as its own process on this backend (see piperTTS.js and
// setupPiper.js for the full explanation). Lets the frontend request
// male or female Arabic voice audio, returned as a base64 WAV data URL
// -- matching the same response shape /transcribe already uses, for
// consistency.
// ------------------------------------------------------------------
app.get("/piper-voices", (req, res) => {
  // Lets the frontend check which voices are genuinely available on
  // THIS deployment before offering them as an option -- the female
  // voice's download source couldn't be fully verified ahead of time
  // (see setupPiper.js), so this is how the frontend finds out for
  // certain rather than assuming.
  res.json({ voices: getAvailableVoices() });
});

app.post("/piper-speak", rateLimitChat, async (req, res) => {
  try {
    ...
    const audioBuffer = await synthesizeArabicSpeech(text.trim(), gender);
