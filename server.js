...
app.post("/chat", rateLimitChat, async (req, res) => {
  try {
    const { message, mode, history, timezone: userTimezone } = req.body;
      ...
      const messages = [
        {
          ...
          content:
            ...
            "GOLD MARKET HOURS -- READ THIS FIRST, HIGHEST PRIORITY RULE (a confirmed real bug this took FIVE attempts to fully fix -- composing correct closed-market phrasing from prose instructions alone kept failing in new subtle ways each time, including a version that avoided the literally banned words but still wrote 'As of now (August 1 at 9:01 PM)... gold's last price was $4,043.21', which is exactly as misleading as saying 'current price'): every single time you call get_gold_prediction, check the gold_market_open field in its result -- true almost all week now (gold's real schedule, corrected from an earlier wrong assumption based on GLD's narrow ETF hours), false only during the Friday-evening-to-Sunday-evening weekend gap. THE FIX: do not try to compose the right closed-market wording yourself -- get_gold_prediction's result now includes price_label, predicted_price_label, and market_closed_statement, pre-written server-side with the exactly correct phrasing for the current market status. Use price_label and predicted_price_label VERBATIM as the labels for current_price_usd and predicted_price_usd, copied exactly, every single time you mention them (including in any summary/bullet section). If market_closed_statement is non-null, it must be the literal first sentence of your response, verbatim, nothing before it. This removes the need to remember or reconstruct the closed-market rules yourself -- just copy the given strings exactly. Also state market_reopens_note exactly as given (already formatted in the user's own local timezone) when present. " +
            "OIL MARKET HOURS -- READ THIS FIRST, HIGHEST PRIORITY RULE (same fix as gold's, applied here too after a confirmed real bug: a response correctly labeled the forecast 'Expected Price When Markets Reopen' but still labeled the OTHER figure 'Current Price' despite oil markets being closed -- an inconsistency that undermines the whole point): every single time you call get_oil_prediction or get_live_oil_price, check the oil_market_open field -- oil trades nearly continuously (Sunday 6 PM ET to Friday 5 PM ET) but ALSO pauses for a genuine 1-hour daily maintenance break (5-6 PM ET, Mon-Thu), a real difference from gold's schedule. Use price_label and predicted_price_label VERBATIM as the labels for current_price_usd and predicted_price_usd, copied exactly, every single time you mention them, with zero exceptions anywhere in your response including summaries. If market_closed_statement is non-null, it must be the literal first sentence of your response, verbatim. Also state market_reopens_note exactly as given (already localized to the user's own timezone) when present. Note oil's current_price_usd is SEPARATELY always a snapshot with a real multi-day data lag regardless of market status (see get_oil_prediction's own detailed instructions for that distinct issue) -- both things apply at once, they are not alternatives to each other. " +
