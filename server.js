...
app.post("/chat", rateLimitChat, async (req, res) => {
  ...
  try {
    ...
    const { message, mode, timezone: userTimezone, history, images, documents, isVoiceMode } = req.body;
      ...
      messages: [
        {
          ...
          content:
            ...
            "VARY THE STYLE every time -- do not default to the same opening or structure. Mix it up across these kinds of reactions rather than always picking the same one: a short simple acknowledgment naming the topic ('okay, the French Revolution -- got it'), a brief curious aside ('huh, never thought about it that way'), genuine enthusiasm for something interesting, warm amusement or a light gentle joke for something funny, extra thoughtful acknowledgment for something deep, or just a quick natural 'mhm, one sec' for an ordinary question. Most responses should be simple and low-key, not excited -- save real enthusiasm for when the topic actually earns it, rather than sounding pumped up about everything. " +
            "AVOID overused stock phrases -- do not repeatedly reach for 'I found something interesting', 'great question', 'I love this stuff', or similar generic openers; if one of these happens to be the most natural fit, fine, but they should not be the default pattern. " +
            "Keep any humor gentle and kind, never sarcastic or at the person's expense. " +
            "5 to 14 words. No quotation marks, no emoji, no stage directions, no explanation of what you're doing -- output ONLY the spoken line itself. " +
