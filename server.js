  ...
  try {
    ...
    if (!answer) {
      ...
      if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
        ...
        // PHRASING CONSISTENCY FIX (a confirmed real bug: GPT correctly
        // used the closed-market label "Expected Price When Markets
        // Reopen" in a bulleted summary, but ELSEWHERE in the very same
        // response reverted to open-market phrasing like "the next
        // hourly update" in its own prose sentence -- inconsistent
        // application within a single reply, not just occasional
        // omission). Rather than trust yet another prompt-only fix for
        // this same recurring category of partial compliance, this
        // corrects it directly in the final text whenever the market is
        // closed, so it can no longer slip through in ANY sentence,
        // bulleted or prose, regardless of how GPT chose to phrase it.
        if (marketClosedPrefix) {
          answer = answer
            .replace(/next hourly update/gi, "when markets reopen")
            .replace(/next trading day/gi, "when markets reopen");
        }
      } else {
