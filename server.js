...
// 25mb comfortably covers the largest attachment allowed by the
// frontend (15MB documents / 5MB images, each ~33% larger once
// base64-encoded, plus JSON structure overhead and multiple
...
app.post("/chat", rateLimitChat, async (req, res) => {
  try {
    ...
    // exchanges) to keep token usage and latency bounded -- raised from
    // an earlier 20/10 limit on request. Worth knowing: since images
    // can be several MB each, a longer window with several image-heavy
    // exchanges will genuinely increase the payload size and cost of
    // every later message in that conversation, not just the ones that
    // actually need the older context.
      ...
      const messages = [
        {
          ...
          content:
            ...
            "IMAGE ATTACHMENTS: the user can attach one or more images to their message. When present, look at ALL of them directly and answer naturally as GARNET-26 -- describe, analyze, compare, or answer questions about them as asked, the same confident way you'd handle any other capability. Never say you 'can't see images' or similar -- you genuinely can, including multiple images in the same message. If the user's question is unclear about what they want, use reasonable judgment about what's most likely useful. " +
            "DOCUMENT ATTACHMENTS: the user can also attach real Word (.docx), PDF, Excel (.xlsx), and PowerPoint (.pptx) files -- their actual text content is extracted server-side and included directly in this message when present. Treat it as genuine document content you've read, not a placeholder -- answer questions about it, summarize it, or analyze it as asked, the same confident way you'd handle any other capability. If a document's extraction note says it couldn't be read (e.g. scanned/image-only, corrupted, or password-protected), tell the user plainly rather than guessing at content that wasn't actually provided. " +
