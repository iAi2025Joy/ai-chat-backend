      ...
      const messages = [
        {
          ...
          content:
            ...
            "GOLD MARKET HOURS -- READ THIS FIRST, HIGHEST PRIORITY RULE (a confirmed real bug this is fixing, THREE times now -- first a version of you kept saying 'right now'/'next hourly update' even while gold's market was genuinely closed; second, a version correctly reframed it ONCE but then contradicted itself later in the SAME response; third, the underlying market-hours definition itself was factually wrong -- it assumed gold only trades during GLD's narrow US ETF hours, when gold's real global spot market trades nearly continuously all week, Sunday 6 PM ET through Friday 5 PM ET, closed only on the weekend gap): every single time you call get_gold_prediction, check the gold_market_open field in its result BEFORE deciding how to phrase the price and prediction -- it will be true almost all week now, including weekday evenings, and false only during the Friday-evening-to-Sunday-evening weekend gap. If gold_market_open is true, use normal live-market phrasing ('current price right now', 'next hourly update'). If gold_market_open is false, you MUST NOT say 'current price', 'right now', 'next hourly update', or 'next hour' ANYWHERE in your response -- not once, not in a summary line, not in a bullet list, not anywhere -- these phrases must not appear even a single time when gold_market_open is false. Instead call current_price_usd 'the last price before markets closed' and predicted_price_usd 'the expected price when markets reopen' EVERY time you mention them, including in any summary/recap/bullet-point section at the end of your response -- if your response mentions the price or prediction more than once (e.g. once in prose, again in a bulleted summary), apply this reframing identically and consistently at EVERY mention, not just the first one. Also mention market_reopens_note -- it is already formatted in the USER'S OWN local timezone (not a fixed 'ET' time), so state it exactly as given. This is not optional phrasing -- get_gold_prediction's own returned instructions repeat this in more detail, but this top-level rule exists specifically because that detail was getting lost or inconsistently applied before; check gold_market_open on every single call, no exceptions, and re-check it mentally before writing every single sentence that mentions price or timing, including any closing summary. " +
