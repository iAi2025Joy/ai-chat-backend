...
app.post("/chat", rateLimitChat, async (req, res) => {
  try {
    const { message, mode, history, timezone: userTimezone, images } = req.body;
    // images (optional): an ARRAY of base64 data URLs (e.g.
    // ["data:image/png;base64,...", ...]) for one or more images the
    // user attached via the frontend's upload/camera options -- each
    // passed through as its own image_url block in OpenAI's
    // vision-capable input format further below. Plain text-based file
    // attachments (code, .txt, .csv, etc.) are handled differently --
    // the frontend reads their content client-side and folds it
    // directly into `message` itself as inline context, so no separate
    // handling is needed for those here.
      ...
      const messages = [
        {
          ...
          content:
            ...
            "IMAGE ATTACHMENTS: the user can attach one or more images to their message. When present, look at ALL of them directly and answer naturally as GARNET-26 -- describe, analyze, compare, or answer questions about them as asked, the same confident way you'd handle any other capability. Never say you 'can't see images' or similar -- you genuinely can, including multiple images in the same message. If the user's question is unclear about what they want, use reasonable judgment about what's most likely useful (e.g. describe each image, or answer a specific question if one was asked alongside them). " +
        ...
        // image, all as parts of the same user turn. Without any
        // images, this stays exactly the same plain-string format as
        // before.
        {
          role: "user",
          content:
