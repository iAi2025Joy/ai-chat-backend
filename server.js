      ...
      const messages = [
        {
          ...
          content:
            ...
            "GOLD MARKET HOURS -- READ THIS FIRST, HIGHEST PRIORITY RULE (a confirmed real bug this is fixing, TWICE now -- first a version of you kept saying 'right now'/'next hourly update' even while gold's market was genuinely closed; then a second version correctly reframed it ONCE in the main paragraph but then contradicted itself later in the SAME response, reverting to 'Predicted Price (Next Hourly Update)' in a summary section below): every single time you call get_gold_prediction, check the us_market_open field in its result BEFORE deciding how to phrase the price and prediction. If us_market_open is true, use normal live-market phrasing ('current price right now', 'next hourly update'). If us_market_open is false, you MUST NOT say 'current price', 'right now', 'next hourly update', or 'next hour' ANYWHERE in your response -- not once, not in a summary line, not in a bullet list, not anywhere -- these phrases must not appear even a single time when us_market_open is false. Instead call current_price_usd 'the last price before markets closed' and predicted_price_usd 'the expected price when markets reopen' EVERY time you mention them, including in any summary/recap/bullet-point section at the end of your response -- if your response mentions the price or prediction more than once (e.g. once in prose, again in a bulleted summary), apply this reframing identically and consistently at EVERY mention, not just the first one. Also mention market_reopens_note so the user knows roughly when. This is not optional phrasing -- get_gold_prediction's own returned instructions repeat this in more detail, but this top-level rule exists specifically because that detail was getting lost or inconsistently applied before; check us_market_open on every single call, no exceptions, and re-check it mentally before writing every single sentence that mentions price or timing, including any closing summary. " +
