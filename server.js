...
// happens.
const START_PHASE_WORDS = ["Thinking", "Identifying what's needed", "Investigating your question"];
const REVIEW_PHASE_WORDS = ["Reviewing what I found", "Weighing the details", "Analyzing what I found", "Investigating further"];
const FINALIZE_PHASE_WORDS = ["Finalizing the response", "Honing the response", "Refining the answer"];

function pickPhaseWord(pool, roundNumber) {
  return pool[roundNumber % pool.length];
}
...
// are about GARNET-26's own identity/data, not an external fact to look
...
// writing, code, simple math, questions about GARNET-26 itself) opt out.
function detectForcedWebSearch(message) {
  if (!message || typeof message !== "string") return null;
  const text = message.toLowerCase().trim();
  if (!text) return null;

  // Skip -- about GARNET-26 itself (identity, its own data/methodology),
...
function getRenderChartToolDefinition() {
  return {
    ...
    function: {
      name: "render_chart",
      description:
        "Render a real chart (line, bar, or pie) for the user using data you actually have -- from a tool result (get_gold_price_history, search_web, fetch_web_page) or genuinely well-known facts. This DIRECTLY renders the chart for the user automatically -- do NOT also try to write a ```chart fenced code block or repeat the raw data yourself; just call this function, then continue your response normally (a short sentence of context is enough, e.g. 'Here's the breakdown:' -- the chart itself appears automatically, you don't need to describe how it could be visualized, just render it). Use 'line' for a trend over time (e.g. price history), 'bar' for comparing several items, 'pie' for parts of a whole/percentages that add up to ~100%. NEVER invent or estimate numbers just to have something to chart -- only call this with real data.",
      parameters: {
        type: "object",
        properties: {
          ...
          labels: {
            type: "array",
            items: { type: "string" },
            description: "Category/x-axis labels, e.g. ['Primary', 'Secondary', 'Tertiary'] or dates for a line chart. Must be the same length as data.",
          },
          data: {
            type: "array",
            items: { type: "number" },
            description: "The real numeric values, same order and same length as labels.",
          },
          yAxisLabel: { type: "string", description: "Optional y-axis label for line/bar charts, e.g. 'USD/oz' or 'Percent'. Omit for pie charts." },
        },
        required: ["title", "type", "labels", "data"],
      },
    },
  };
}
...
function handleRenderChartCall(argsJson) {
  ...
  if (!Array.isArray(labels) || !Array.isArray(data) || labels.length === 0 || labels.length !== data.length) {
    console.error(
      "render_chart: validation failed (labels/data mismatch).",
    ...
    return {
      toolResult: JSON.stringify({ error: "labels and data must be non-empty arrays of the same length." }),
      chartHtml: null,
    };
  }
...
app.post("/chat", rateLimitChat, async (req, res) => {
  ...
  try {
    sendEvent({ status: pickPhaseWord(START_PHASE_WORDS, 0) });

    const { message, mode, timezone: userTimezone, history, images, documents } = req.body;
    ...
    if (!answer) {
      ...
      const messages = [
        {
          ...
          content:
            ...
            "IMAGE ATTACHMENTS: the user can attach one or more images to their message. When present, look at ALL of them directly and answer naturally as GARNET-26 -- describe, analyze, compare, or answer questions about them as asked, the same confident way you'd handle any other capability. Never say you 'can't see images' or similar -- you genuinely can, including multiple images in the same message. If the user's question is unclear about what they want, use reasonable judgment about what's most likely useful. If an image is a government-issued ID, passport, or similar personal identity document, don't extract or discuss the personal details on it (name, ID/passport number, date of birth, etc.) -- decline specifically and clearly along these lines, in your own words rather than a generic refusal: \"I'm not able to read or share details from ID cards, passports, or similar personal documents. Happy to help with anything else, though -- other images, documents, or your gold/oil questions!\" " +
        ...
        ...safeHistory,
        // When one or more images are attached, the message content
        // becomes an array (OpenAI's vision input format) instead of a
        // plain string -- one text block plus one image_url block PER
        // image. Without any images, this stays a plain string (now
        // effectiveMessage, which includes any extracted document text
        // folded in above).
        {
          role: "user",
          content:
      ...
      if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
        ...
        while (responseMessage.tool_calls && responseMessage.tool_calls.length > 0 && toolRound < MAX_TOOL_ROUNDS) {
          ...
          for (const toolCall of responseMessage.tool_calls) {
            sendEvent({ status: TOOL_STATUS_LABELS[toolCall.function.name] || pickPhaseWord(REVIEW_PHASE_WORDS, toolRound) });
          ...
          sendEvent({ status: pickPhaseWord(REVIEW_PHASE_WORDS, toolRound) });
        ...
        if ((!responseMessage.content || !responseMessage.content.trim()) && toolRound >= MAX_TOOL_ROUNDS) {
          sendEvent({ status: pickPhaseWord(FINALIZE_PHASE_WORDS, 0) });
      ...
      // reaches the user.
      const chartWasRequested = /\b(draw|plot|chart|graph|visuali[sz]e)\b/i.test(message || "");
      if (chartWasRequested && renderedChartBlocksForResponse.length === 0) {
        ...
        while (correctionMessage.tool_calls && correctionMessage.tool_calls.length > 0 && correctionRounds < 2) {
          ...
          sendEvent({ status: TOOL_STATUS_LABELS[toolCall.function.name] || pickPhaseWord(REVIEW_PHASE_WORDS, correctionRounds) });
            let toolResult;
        ...
        if ((!correctionMessage.content || !correctionMessage.content.trim()) && correctionMessage.tool_calls && correctionMessage.tool_calls.length > 0) {
          sendEvent({ status: pickPhaseWord(FINALIZE_PHASE_WORDS, 1) });
        ...
        if (renderedChartBlocksForResponse.length === 0) {
          ...
          if (forcedMessage.tool_calls && forcedMessage.tool_calls.length > 0) {
            ...
            sendEvent({ status: pickPhaseWord(FINALIZE_PHASE_WORDS, 2) });
