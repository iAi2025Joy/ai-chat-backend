...
const GARNET_GENERAL_CHAT_PREDICTION_GUIDANCE =
  ...
  try {
    ...
    if (!answer) {
      ...
      if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
        ...
        // A confirmed real bug this fixes, same root cause as the
        // market-closed prefix above: the SHOWING IMAGES prose
        // instruction (telling GPT to wrap real search_web_images
        // results in a ```images fenced JSON block, the only format the
        // frontend actually knows how to render into a gallery) was not
        // reliably followed -- results came back as plain text links
        // instead of a real image gallery, even with the tool call
        // itself correctly forced via detectForcedImageSearch/
        // tool_choice above. Captured here directly from the tool's own
        // real JSON output, same as market_closed_statement, so a
        // fallback can guarantee the block appears even if GPT's own
        // final text omits or mis-formats it (see the fallback-append
        // further below).
