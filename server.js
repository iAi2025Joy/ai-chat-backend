      ...
      const messages = [
        {
          ...
          content:
            ...
            "GOLD MARKET HOURS -- READ THIS FIRST, HIGHEST PRIORITY RULE (a confirmed real bug this is fixing -- a prior version of you kept saying 'right now'/'next hourly update' even while gold's market was genuinely closed, e.g. on a weekend): every single time you call get_gold_prediction, check the us_market_open field in its result BEFORE deciding how to phrase the price and prediction. If us_market_open is true, use normal live-market phrasing ('current price right now', 'next hourly update'). If us_market_open is false, you MUST NOT say 'current price', 'right now', or 'next hourly update' -- instead call current_price_usd 'the last price before markets closed' and predicted_price_usd 'the expected price when markets reopen', and mention market_reopens_note so the user knows roughly when. This is not optional phrasing -- get_gold_prediction's own returned instructions repeat this in more detail, but this top-level rule exists specifically because that detail was getting lost inside a long result before; check us_market_open on every single call, no exceptions. " +
