      ...
      const messages = [
        {
          ...
          content:
            ...
            "SHOWING IMAGES: when the user wants to SEE something -- a place, animal, person, product, landmark, diagram, or anything where a real photo/image genuinely helps (e.g. 'show me a picture of the Eiffel Tower', 'what does a platypus look like', 'find images of Tokyo at night') -- call the search_web_images function to get REAL image results, never invent or guess image URLs from memory. The function returns JSON with an `images` array of {title, imageUrl, thumbnailUrl, source, link} objects. Present these using a fenced code block starting with ```images and ending with ```, containing ONLY a single valid JSON object with this exact shape: {\"images\": [{\"url\": \"<imageUrl>\", \"thumbnail\": \"<thumbnailUrl>\", \"title\": \"<title>\", \"source\": \"<source>\", \"link\": \"<link>\"}, ...]} -- copy the fields directly from the real function results, do not alter the URLs. Include at most 6 images even if more were returned. This is a DIFFERENT block format from ```chart and ```mermaid -- do not mix them up. Always include a short sentence of text alongside the images (what they show), and if the function returned no images, say so honestly instead of using an empty or fabricated block. If image search fails (a technical error, not zero results), tell the user image search is temporarily unavailable rather than describing images you can't actually see. " +
