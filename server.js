      ...
      const messages = [
        {
          ...
          content:
            ...
            "PRICE CHARTS: when the user wants to SEE gold's recent price trend as a chart/graph/line diagram (e.g. 'draw a line chart of gold prices for the last 24 hours', 'show me how gold moved today', 'plot the last day's prices'), call the get_gold_price_history function first to get REAL data -- never fabricate price history from memory. Then present it using a fenced code block starting with ```chart and ending with ```, containing ONLY a single valid JSON object with this exact shape: {\"title\": \"Gold Price - Last 24 Hours (USD/oz)\", \"labels\": [\"Jul 22, 14:00\", \"Jul 22, 15:44\", ...], \"data\": [4126.93, 4131.53, ...]} -- labels and data must be the same length and in the same order as the real points returned by the function. This is a DIFFERENT tool and DIFFERENT block format from the prediction tool and the mermaid diagrams above -- do not mix them up. For gold price trend charts specifically, only use real data returned by get_gold_price_history, never fabricated numbers. Always include a short sentence of text alongside the chart (e.g. the actual date range it covers, and a note that this is historical data, not a prediction). " +
            "GENERAL DATA CHARTS (bar/pie): the same ```chart fenced block format above also supports bar and pie charts for OTHER real data you've gathered (via search_web, fetch_web_page, or well-established general knowledge) that's naturally categorical or proportional -- e.g. market share breakdown, a comparison across several items, percentages that add up to a whole. Add an optional \"type\" field set to \"bar\" or \"pie\" (omit it, or use \"line\", for a trend-over-time line chart like the gold price case above): {\"title\": \"...\", \"type\": \"pie\", \"labels\": [\"Category A\", \"Category B\", ...], \"data\": [40, 60, ...]}. Only chart REAL numbers you actually have from a tool result or genuinely well-known facts -- never invent plausible-looking figures just to produce a chart; if you don't have real numbers to chart, present the information as text/table/list instead. Choose whichever of paragraphs, bullet points, a table, or a chart actually fits the data and the question -- not the same format every time. " +
