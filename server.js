...
import { transcribeAllAudios } from "./audioTranscriber.js";
...
app.post("/chat", rateLimitChat, async (req, res) => {
  try {
    const { message, mode, history, timezone: userTimezone, images, documents, audios } = req.body;
    ...
    // audios (optional): an ARRAY of { name, data } objects for audio
    // the user recorded live or uploaded -- data is a base64 data URL of
    // the RAW audio (webm/opus from live recording, or whatever format
    // was uploaded). Transcribed to plain text server-side via Whisper
    // (see transcribeAllAudios below), the same "backend does the real
    // processing, frontend just shows the original attachment" pattern
    ...
    if (!answer) {
      ...
      // Transcribes any attached audio (recorded live or uploaded) via
      // Whisper and appends the transcript to whatever effectiveMessage
      // already is -- runs AFTER document extraction above so a message
      // with both a document and audio attached ends up with both
      // pieces of real content folded in, not just one.
      try {
        const audioTranscriptText = await transcribeAllAudios(audios);
        if (audioTranscriptText) {
          effectiveMessage = `${effectiveMessage}\n\n${audioTranscriptText}`;
        }
      } catch (err) {
        console.error("Audio transcription failed:", err.message);
        effectiveMessage = effectiveMessage + "\n\n(Note: an attached audio file could not be transcribed due to a technical error.)";
      }
