      ...
      const messages = [
        {
          ...
          content:
            ...
            "FEDERAL RESERVE INTEREST RATE QUESTIONS -- READ THIS FIRST, HIGHEST PRIORITY RULE (a confirmed real bug this is fixing -- a prior version of you answered a Fed rate question from stale training data instead of calling the tool, giving a wrong rate and a wrong year): if the user asks ANYTHING about the current Federal Reserve / Fed interest rate, the Fed funds rate, or what the Fed is expected to do with rates in the future -- including questions that do NOT mention 'dollar', 'DXY', or 'Dollar Index' at all, e.g. 'what's the current Fed rate', 'what interest rate does the Fed expect', 'will the Fed cut rates', 'what is the federal funds rate' -- you MUST call the get_dxy_prediction function and answer using ONLY its real returned fields (current_fed_funds_rate_lower_pct, current_fed_funds_rate_upper_pct, fed_rate_outlook). This data is NOT in your training data, changes over time, and your training data on this topic is guaranteed to be outdated -- NEVER answer a Fed interest rate question from memory/general knowledge under any circumstances, and never fabricate source links for it either. This rule applies even if the question sounds like general economic knowledge -- if the topic is the Fed's interest rate, current or future, always call the tool first. " +
      ...
      if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
        ...
        for (const toolCall of responseMessage.tool_calls) {
          ...
          if (toolCall.function.name === "get_gold_prediction") {
            ...
            toolResult = await handleDxyPredictionCall();
          } else {
