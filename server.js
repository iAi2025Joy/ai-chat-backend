      ...
      const messages = [
        {
          ...
          content:
            ...
            "MATH FORMULAS -- READ THIS FIRST, HIGHEST PRIORITY RULE (a confirmed real bug this fixes: outside School and Students mode, equations were inconsistently written as plain text like \"F = ma\" or \"x^2\" with no math delimiters at all, so nothing rendered as real typeset math in the chat UI, even though the exact same equation typeset correctly when the SAME model wrote it inside a generated PDF -- the underlying capability was never the problem, this rule just wasn't being followed reliably enough in normal chat): whenever you solve or present ANY mathematical formula or equation, in ANY mode, you MUST wrap it in real math delimiters -- inline math in \\( and \\), standalone/display equations in \\[ and \\] on their own line -- NEVER as plain inline text with no delimiters (do NOT use single or double dollar signs as delimiters either). This is not optional formatting polish -- an equation written without these delimiters simply does not render as math at all, it shows up as raw, unformatted text, which is a genuine failure to answer well, not a minor stylistic choice. Use real LaTeX math notation throughout, not plain-text approximations -- proper fractions (\\frac{}{}), exponents (x^2), square roots (\\sqrt{}), Greek letters (\\alpha, \\pi), summation/integral signs (\\sum, \\int), subscripts (x_1), comparison operators (\\leq, \\times), etc. -- rather than typing things like \"x^2\" or \"sqrt(x)\" as plain text outside any delimiter. For a step-by-step solution, give each step its own **bold** step label (e.g. **Step 1: Isolate x**) followed by the real, properly-delimited math for that step -- one bold label per step, not one giant unlabeled block. Never describe an equation in words instead of actually writing it with real math notation. " +
