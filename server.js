...
function isLongHorizonHistoricalQuery(text) {
  return (
    /\b(last|past)\s+\d+\s*\+?\s*(years?|decades?)\b/.test(text) ||
    /\b\d+\s*-?\s*years?\s+(history|trend|chart|data|prices?)\b/.test(text) ||
    /\bdecade(s)?\b/.test(text) ||
    /\bhistor(y|ical)\b/.test(text) ||
...
function detectForcedWebSearch(message) {
  ...
  // Long-horizon historical data requests (e.g. "gold prices during last
  // 10 years") -- neither get_gold_prediction nor get_gold_price_history
  // (24-hour window only) can answer these, so force a real web search
  // instead of letting the model decline or point the user elsewhere.
  // See the matching comment on detectForcedPredictionTool above.
  if (isLongHorizonHistoricalQuery(text)) return "search_web";

  if (/^\s*(where|who|when)\s+(is|are|was|were|did|does|do)\b/.test(text)) return "search_web";
  if (/^\s*how\s+(many|much|old|far|tall|big|long)\b/.test(text)) return "search_web";

  // A confirmed real bug this fixes: "give me the names of the current
  // DPhil students in Biology at Oxford" matched none of the WH-question
  // patterns above (it's an imperative, not a "who/where/when" question),
  // so no tool was forced and the model answered from ungrounded memory
  // (or possibly a real search that still wasn't reliably triggered)
  // instead of definitely searching. Broadens coverage to the other
  // common ways people phrase a factual-lookup request. Excludes
  // obviously creative/non-factual asks (a poem, a joke, a story, code,
  // a draft) that happen to start with the same verbs -- those aren't
  // requests for an external fact and shouldn't force a search.
  if (/^\s*(give\s+me|list|find(\s+me)?|tell\s+me|name|identify|provide(\s+me\s+with)?|get\s+me)\b/.test(text)) {
    if (!/\b(poem|story|joke|song|lyrics?|essay|letter|email\s+draft|code|script|recipe)\b/.test(text)) {
      return "search_web";
    }
  }
...
app.post("/chat", rateLimitChat, async (req, res) => {
  try {
    const { message, mode, history, timezone: userTimezone, images, documents } = req.body;
