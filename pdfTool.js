...
export function getCreatePdfToolDefinition() {
  return {
    ...
    function: {
      name: "create_pdf",
      description:
        "Create a real, downloadable single PDF file. Use this when the user explicitly asks for a PDF (not a LaTeX/Overleaf project -- use create_project_zip for that when they explicitly ask for the LaTeX source itself). Best for reports, summaries, or documents made of headings, paragraphs, and simple tables -- NOT for documents that need real typeset mathematical notation or a specific named academic venue's exact two-column format (USENIX, IEEE, ACM, etc.), since this produces plain formatted text/tables, not real LaTeX typesetting -- use create_latex_pdf instead for those, which actually compiles real LaTeX into a real PDF and still gives the person the PDF file type they asked for. For a research paper, report, or other document where the person asked for something \"full\", \"detailed\", \"complete\", or \"professional\", follow the MATCH THE REAL DEPTH/LENGTH checklist defined elsewhere in this system exactly (real literature review, analysis/discussion, ethics section where it applies, tables, a genuine conclusion, real references) -- that checklist applies to a PDF exactly the same way it applies to a LaTeX project, not a lighter version of it. Never present invented/illustrative numbers as if they were real measured results -- see the NEVER PRESENT FABRICATED EXPERIMENTAL DATA rule elsewhere in this system, which applies here too. This DIRECTLY renders a real download card for the user automatically -- do not also paste the content again as a code block or repeat it in your text. After calling this, continue your response normally with a short sentence of context -- you don't need to repeat the content, the card already lets them download and open it.",
      parameters: {
        type: "object",
        properties: {
