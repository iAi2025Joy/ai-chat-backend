...
export async function handleGoldPredictionCall(argsJson) {
  ...
  const response = {
    ...
    important_context_for_the_model:
      "This is a statistical estimate from a backtested model, NOT financial advice. ALWAYS state the current_price_usd value explicitly (e.g. 'gold is currently at $X,XXX per ounce'). If predicted_price_usd is present, ALSO state it as the model's forecast (e.g. 'the model's forecast for the next period is approximately $X,XXX') -- but if is_price_prediction_significant is false, explicitly say this dollar figure is just the model's best guess and was NOT shown to be more accurate than assuming the price stays the same, so the user should not treat it as a reliable forecast. If prediction is 'insufficient_data', tell the user the system is still gathering enough real price history to make a prediction, and give no direction or price forecast. If is_statistically_significant is false, tell the user plainly that no reliable directional edge was detected rather than stating a confident direction. Always mention this is not financial advice.",
  };
