...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  const response = {
    ...
    important_context_for_the_model:
      "This is a statistical estimate from a backtested model, NOT financial advice. MARKET HOURS AWARENESS (corrected -- an earlier version of this wrongly assumed gold only trades during GLD's own US equity-ETF hours; gold's actual global spot market trades nearly continuously all week): gold trades essentially 24 hours a day from Sunday 6:00 PM ET through Friday 5:00 PM ET, closed only during the weekend gap (Friday 5:00 PM ET to Sunday 6:00 PM ET). Check gold_market_open FIRST, before deciding how to phrase anything about price or prediction. If gold_market_open is true, use the normal framing below ('next hourly update', 'current price right now', etc.) exactly as described -- this will be true almost all week, including weekday evenings/nights, since gold genuinely keeps trading through those hours via the Asian and European sessions. If gold_market_open is FALSE (only during the Friday-evening-to-Sunday-evening weekend gap), you MUST reframe: (1) call current_price_usd 'the last price before markets closed', NOT 'the current price' or 'right now' -- markets aren't moving right now, so implying a live, ticking price is misleading even though the number itself is accurate; (2) call predicted_price_usd 'the expected price when markets reopen' instead of 'the next hourly forecast' -- there is no 'next hour' to speak of while markets are shut, so that phrase would be actively wrong, not just imprecise; (3) mention market_reopens_note (already formatted in the user's own local timezone -- use it exactly as given, do not reformat or convert it yourself) so the user knows exactly when trading resumes in terms meaningful to them; (4) still apply all the same significance/best-guess/disagreement caveats below exactly as normal -- only the PRICE-related phrasing changes, not the honesty standards; (5) CRITICAL, a confirmed real bug -- apply this reframing at EVERY mention of price or prediction in your response, not just the first one. A prior response correctly said 'expected price when markets reopen' once in its opening paragraph, then contradicted itself later in a summary/bullet section by reverting to 'Predicted Price (Next Hourly Update)'. If your response has a summary, recap, or bullet-point section, it must use the exact same closed-market phrasing as the rest of the response, with zero exceptions. " +
