...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  if (predictionForNote === "up" || predictionForNote === "down") {
    const probUp = freshPrice.liveInferenceUsed ? freshPrice.liveInferenceProbabilityUp : (data.prediction_probability_up ?? null);
    const directionConfidencePct = typeof probUp === "number"
      ? (predictionForNote === "up" ? probUp * 100 : (1 - probUp) * 100)
      : null;
    ...
    let strengthWord = "a slight lean";
    if (directionConfidencePct !== null) {
      if (directionConfidencePct >= 75) strengthWord = "strong confidence";
      else if (directionConfidencePct >= 60) strengthWord = "moderate confidence";
      else strengthWord = "only a slight lean, close to a coin flip";
    }
    const confClause = directionConfidencePct !== null
      ? ` (${strengthWord}, ${directionConfidencePct.toFixed(1)}%)`
      : "";

    unifiedPredictionHeadline =
      `Gold is currently at ${currentStr}. The model expects it to move ${predictionForNote}${confClause} over the next hourly update, with a rough price target around ${predictedStr}.`;
  }
  ...
  const response = {
    ...
    important_context_for_the_model:
      "MANDATORY OPENING SENTENCE -- READ THIS FIRST, a real instruction that has now failed repeatedly despite multiple prose-only attempts (including explicit bad-vs-good examples), up to and including a directly self-contradictory live response stating 'increase' and 'leaning down' in the same reply: unified_prediction_headline is a complete sentence ALREADY WRITTEN FOR YOU, assembled directly from the real numbers so it cannot be self-contradictory. When gold_market_open is true, use it EXACTLY AS GIVEN, verbatim, word for word, as the very first sentence of your response, with nothing else before it -- do not paraphrase it, shorten it, reorder it, or compose your own version. This field is in English regardless of the conversation's language -- state it exactly as given even if the rest of your response is in a different language (this one sentence being English is a deliberate, known trade-off for guaranteed correctness, better than a fluent but self-contradictory sentence in the right language). When gold_market_open is false, use market_closed_statement as the first sentence instead (see MARKET HOURS below) -- the two are mutually exclusive, never use both. Nothing else about the price/direction/confidence combination belongs anywhere else in your response after this -- no second paragraph, no bullet, no 'Note:', no restating the numbers again in different words. " +
      ...
      "COIN-FLIP STRENGTH WORDING is already built into unified_prediction_headline above (it automatically says 'strong confidence', 'moderate confidence', or 'only a slight lean, close to a coin flip' based on the real number) -- do not add your own additional coin-flip commentary on top of it. " +
