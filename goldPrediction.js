...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  // DETERMINISTIC UNIFIED HEADLINE -- a confirmed real, REPEATED
  // failure this fixes: prose instructions asking GPT to compose ONE
  // combined direction+price sentence, even with explicit bad-vs-good
  // examples and a "read this first" mandate, still produced visibly
  // self-contradictory output in production (a live response stated
  // "gold is likely to increase" alongside "prices are leaning down" in
  // the same reply). Same fix as market_closed_statement above, applied
  // to a second recurring failure point: stop asking GPT to compose
  // this from scratch every time -- build it directly from the real
  // numbers in code, where a contradiction is structurally impossible,
  // and require GPT to use it verbatim rather than write its own
  // version. English-only (like market_closed_statement) -- a real,
  // known trade-off of this technique, but full sentence-level
  // reliability across every language isn't achievable via prose
  // instructions alone, and this specific bug (a directly
  // self-contradictory response) is worse than an English-only opening
  // sentence would be.
  ...
  if (predictionForNote === "up" || predictionForNote === "down") {
    ...
    const confClause = directionConfidencePct !== null
      ? `, though the direction model's own separate confidence in "${predictionForNote}" is only about ${directionConfidencePct.toFixed(1)}%${directionConfidencePct < 60 ? " -- closer to a coin flip than a confident trend" : ""}`
      : "";
    unifiedPredictionHeadline =
      `Gold is currently at ${currentStr}, and the price-target model's best estimate for the next hourly update is ${predictedStr} (${freshPrice.priceDirectionVsCurrent || "about the same"} than now)${confClause}.`;
  }
