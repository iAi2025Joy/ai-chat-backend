...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  // Pre-written, ready-to-use rolling-accuracy sentence -- same reliability
  // pattern already used for market_closed_statement/price_label above.
  // MUST be surfaced on every response regardless of whether the user asked
  // about accuracy specifically (confirmed real gap: a real response gave a
  // full prediction with zero mention of live track-record accuracy, since
  // the old instruction only said to use these fields "if the user asks").
  // Computed here, once, as one deterministic sentence GPT can drop in
  // verbatim, rather than trusting it to compose the percentage correctly
  // from the raw numeric fields every single time.
  const trackedCount = data.rolling_predictions_tracked ?? 0;
  const rollingAccuracy = data.rolling_direction_accuracy;
  let rollingAccuracyStatement;
  if (trackedCount > 0 && rollingAccuracy !== null && rollingAccuracy !== undefined) {
    const pct = (rollingAccuracy * 100).toFixed(1);
    rollingAccuracyStatement =
      `Live tracked accuracy so far: ${pct}% correct on direction over the last ${trackedCount} resolved ` +
      `prediction${trackedCount === 1 ? "" : "s"} (checked automatically, hour by hour, against real market prices).` +
      (data.rolling_accuracy_is_significant
        ? " This is statistically distinguishable from a coin flip (binomial test, p<0.05) -- a real, if modest, live edge."
        : " This is not yet statistically distinguishable from a coin flip -- treat it as an early, unproven track record, not a demonstrated edge.");
  } else {
    rollingAccuracyStatement =
      "Live tracked accuracy: no resolved predictions yet. The system automatically checks each hourly prediction " +
      "against the real price an hour later and is building up a real accuracy track record from scratch -- check back soon.";
  }

  // Pre-written disagreement sentence -- same rationale: a real response
  // showed 'Direction: Up' / '67.9% probability of upward movement' next to
  // 'indicates a potential decrease' with no explanation, because the old
  // instruction left composing the disagreement callout entirely to prose
  // discipline. This forces a single, unambiguous, always-separate sentence.
  const disagreementStatement =
    freshPrice.directionPriceAgreement === false
      ? "Note: the model's direction call and its specific price estimate do not agree with each other right now -- " +
        "they come from two separate models and can point different ways; treat this as an extra reason for caution, not an error to paper over."
      : null;
  ...
  const response = {
    ...
    disagreement_statement: disagreementStatement,
    rolling_accuracy_statement: rollingAccuracyStatement,
    ...
    important_context_for_the_model:
      "This is a statistical estimate from a backtested model, NOT financial advice. " +
      "MANDATORY, EVERY SINGLE RESPONSE, EVEN IF NOT ASKED (a confirmed real gap this fixes -- a real response gave a full prediction with zero mention of the live accuracy track record, because the old instruction only surfaced it when the user asked about accuracy specifically): you MUST include rolling_accuracy_statement verbatim, word for word, in every response this tool produces, regardless of what the user actually asked -- do not paraphrase it, do not shorten it, do not wait to be asked about accuracy, and do not omit it even in a short answer. Place it as its own clearly separated sentence or bullet (e.g. a 'Live Tracked Accuracy' line), not buried mid-paragraph. This is the system's own real, continuously-updating scorecard of past predictions checked against actual market outcomes -- keep stating it plainly even while it still says 'no resolved predictions yet', since that itself is the honest current answer, not a placeholder to skip. " +
      "MANDATORY WHEN PRESENT: if disagreement_statement is non-null, you MUST include it verbatim as its own separate, clearly stated sentence -- never let a directional word like 'increase'/'decrease' anywhere else in your response contradict the 'prediction' field or price_direction_vs_current without this statement immediately clarifying why (a confirmed real bug: a response labeled the call 'Direction: Up' with '67.9% probability of upward movement' while also saying 'indicates a potential decrease' elsewhere, with no explanation of the contradiction). " +
      "MARKET HOURS -- READ THIS CAREFULLY, a repeatedly-failed instruction this fixes differently: gold trades essentially 24 hours a day from Sunday 6:00 PM ET through Friday 5:00 PM ET, closed only during the weekend gap. Composing the right closed-market phrasing yourself has failed multiple times in production, including subtle workarounds like writing 'as of now... the last price was $X' (still misleading, since pairing any present-moment phrase with the price recreates the same false impression even while technically avoiding the literally banned words). So: DO NOT compose your own phrasing for this -- use price_label and predicted_price_label EXACTLY AS GIVEN, verbatim, as the literal labels for current_price_usd and predicted_price_usd (e.g. write '**Last Price Recorded Before Markets Closed:** $4,043.21', copying price_label's exact text as the label, when gold_market_open is false -- these two fields already say the correct thing for the current market status, so copying them removes the need to get the phrasing right yourself). If market_closed_statement is non-null, it MUST be the very first sentence of your response, stated exactly as given, verbatim, with nothing else before it. If market_closed_statement is null (market is open), write normally with no special statement needed. Also mention market_reopens_note when present (already formatted in the user's own local timezone -- state it exactly as given, do not reformat or convert it yourself). Apply price_label/predicted_price_label consistently at EVERY mention throughout your entire response, including any summary/recap/bullet section -- not just the first mention. Still apply all the same significance/best-guess/disagreement caveats below exactly as normal regardless of market status. " +
