...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  const response = {
    ...
    important_context_for_the_model:
      "STOP -- READ THIS ONE INSTRUCTION FIRST, before anything else in this note, a real instruction that has now failed twice despite being explained in prose: write your ENTIRE prediction as exactly ONE sentence or two at most, in your own words in the user's language, but structurally like this pattern -- [current price], then [the price estimate] woven into the SAME clause with a direction word, then [the direction model's own confidence] as a short trailing clause in that SAME sentence. Nothing about this topic exists anywhere else in your response after this one sentence -- no second paragraph, no bullet, no 'Note:', no restating the numbers again. " +
      "BAD (a confirmed real, reproduced failure -- do NOT structure it like this, with the current/predicted price as one claim and the direction+confidence as a separate trailing claim glued on after with 'however'): 'The Current Price of gold is $4,389.89. The model's best estimate for the Predicted Price is approximately $4,393.75. The model's directional prediction indicates a movement down; however, it has a confidence level of only about 40%.' " +
      "GOOD (one single woven sentence, direction and price and confidence all inside the same clause, not sequential separate claims): 'Gold is at $4,389.89 right now, and the model's best guess for the next update is around $4,394 -- a slight uptick, though its own directional read leans down with only about 40% confidence, so this is closer to a coin flip than a real trend.' " +
      "Everything else in this note (accuracy stats, data sources, market hours, etc.) still belongs in your response as usual, AFTER this one sentence -- only the price/direction/confidence combination itself must be this one unified sentence, not those other topics. " +
