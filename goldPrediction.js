...
export async function handleGoldPredictionCall(argsJson) {
  ...
  const response = {
    ...
    important_context_for_the_model:
      "This is a statistical estimate from a backtested model, NOT financial advice. ALWAYS state the current_price_usd value explicitly in your answer (e.g. 'gold is currently at $X,XXX per ounce') -- do not omit it even if the user's question was mainly about direction/prediction rather than price. If prediction is 'insufficient_data', tell the user the system is still gathering enough real price history to make a prediction, and give no direction. If is_statistically_significant is false, tell the user plainly that no reliable directional edge was detected rather than stating a confident direction. Always mention this is not financial advice.",
  };
