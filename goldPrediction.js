...
// ------------------------------------------------------------------
// MARKET HOURS -- gold's price data here tracks GLD (an ETF on NYSE
// Arca), which only trades 9:30 AM - 4:00 PM Eastern Time, Mon-Fri.
// Outside those hours, "next hour prediction" doesn't really make
// sense (there is no next trading hour until the market reopens) --
// this is used to reframe the response honestly when markets are
// closed, instead of implying an hourly forecast that can't happen.
// NOTE: does not account for US market holidays (e.g. Thanksgiving,
// Christmas) -- a real, acknowledged simplification; the weekday/time
// check alone covers the large majority of "closed" cases correctly.
// ------------------------------------------------------------------

function getUSMarketStatus(nowUtc = new Date()) {
  // Convert to America/New_York wall-clock time regardless of the
  // server's own timezone (Render runs in UTC, but this needs to be
  // correct regardless of where it's ever hosted).
  ...
  const hour = parseInt(nyParts.find((p) => p.type === "hour").value, 10);
  const minute = parseInt(nyParts.find((p) => p.type === "minute").value, 10);
  const minutesSinceMidnight = hour * 60 + minute;

  const isWeekday = !["Sat", "Sun"].includes(weekday);
  const marketOpenMinutes = 9 * 60 + 30; // 9:30 AM
  const marketCloseMinutes = 16 * 60; // 4:00 PM
  const isDuringHours = minutesSinceMidnight >= marketOpenMinutes && minutesSinceMidnight < marketCloseMinutes;

  const isOpen = isWeekday && isDuringHours;

  // A short, honest, human-readable description of when it reopens --
  // deliberately approximate (doesn't account for holidays), phrased
  // to reflect that.
  let nextOpenDescription;
  if (isOpen) {
    nextOpenDescription = null;
  } else if (weekday === "Sat") {
    nextOpenDescription = "9:30 AM ET on Monday (assuming no market holiday)";
  } else if (weekday === "Sun") {
    nextOpenDescription = "9:30 AM ET on Monday (assuming no market holiday)";
  } else if (weekday === "Fri" && minutesSinceMidnight >= marketCloseMinutes) {
    nextOpenDescription = "9:30 AM ET on Monday (assuming no market holiday)";
  } else if (minutesSinceMidnight >= marketCloseMinutes) {
    nextOpenDescription = "9:30 AM ET tomorrow (assuming no market holiday)";
  } else {
    nextOpenDescription = "9:30 AM ET today";
  }

  return { isOpen, nextOpenDescription };
}
...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  const response = {
    ...
    us_market_open: marketStatus.isOpen,
    market_reopens_note: marketStatus.nextOpenDescription,
    ...
    important_context_for_the_model:
      "This is a statistical estimate from a backtested model, NOT financial advice. MARKET HOURS AWARENESS (a real, deliberate change): gold's price data here tracks GLD, an ETF that only trades 9:30 AM - 4:00 PM Eastern Time, Monday-Friday. Check us_market_open FIRST, before deciding how to phrase anything about price or prediction. If us_market_open is true, use the normal framing below ('next hourly update', 'current price right now', etc.) exactly as described. If us_market_open is FALSE (markets closed -- evenings, weekends), you MUST reframe: (1) call current_price_usd 'the last price before markets closed', NOT 'the current price' or 'right now' -- markets aren't moving right now, so implying a live, ticking price is misleading even though the number itself is accurate; (2) call predicted_price_usd 'the expected price when markets reopen' instead of 'the next hourly forecast' -- there is no 'next hour' to speak of while markets are shut, so that phrase would be actively wrong, not just imprecise; (3) mention market_reopens_note so the user knows roughly when trading resumes (e.g. 'markets reopen at 9:30 AM ET tomorrow'); (4) still apply all the same significance/best-guess/disagreement caveats below exactly as normal -- only the PRICE-related phrasing changes, not the honesty standards; (5) CRITICAL, a confirmed real bug -- apply this reframing at EVERY mention of price or prediction in your response, not just the first one. A prior response correctly said 'expected price when markets reopen' once in its opening paragraph, then contradicted itself later in a summary/bullet section by reverting to 'Predicted Price (Next Hourly Update)'. If your response has a summary, recap, or bullet-point section, it must use the exact same closed-market phrasing as the rest of the response, with zero exceptions. " +
