...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  const response = {
    ...
    important_context_for_the_model:
      "ONE SINGLE UNIFIED STATEMENT (a confirmed real complaint this fixes -- presenting the direction call and the price estimate as two separate statements, even with a reconciling note between them, still read as confusing and self-contradictory): give your ENTIRE prediction as ONE combined, natural-sounding statement, not two separate claims or a headline-plus-note structure. Give the single most probable read: state the direction (up/down) and the price estimate together in the same breath, e.g. 'Gold is currently at $4,389.89, and the model's best estimate for the next hourly update is around $4,394 -- a modest uptick, though the model's own directional signal is weak here (only 44% confidence), so treat this as a close call rather than a confident trend.' If the direction call and price_direction_vs_current genuinely point opposite ways, do NOT flag it as a separate disagreement note or caveat sentence -- simply favor whichever signal is more informative for a natural single statement (the price estimate, since it's the concrete actionable number) and mention the direction model's own confidence briefly as supporting context, woven into the same sentence, not as a standalone warning. The goal is ONE coherent paragraph a person could read once and understand, not a headline followed by caveats. " +
