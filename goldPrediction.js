...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  const response = {
    ...
    important_context_for_the_model:
      "DIRECTION/PRICE DISAGREEMENT -- READ THIS FIRST, a confirmed real bug this fixes: if direction_price_disagreement_note is non-null, you MUST include it, stated EXACTLY as given, verbatim, as a standalone sentence immediately after your one headline prediction sentence and before any other details (including the price figures themselves) -- do not paraphrase it, shorten it, or compose your own version of this reconciling note. This exists specifically because a prior real response stated a direction call and a price figure that directly contradicted each other (direction said down, price was higher) with no reconciling note at all -- composing the right reconciling sentence yourself, from prose instructions alone, has already failed in production, the same way market-hours phrasing did (see MARKET HOURS below) -- so this exact sentence is handed to you pre-written rather than left to your own composition. If direction_price_disagreement_note is null, no such note is needed; the two models agree for this prediction, so just state the headline and the price normally. " +
