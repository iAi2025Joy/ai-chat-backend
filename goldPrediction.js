...
function isGoldMarketOpenAt(checkDate) {
  ...
  // % 24 normalizes a real, confirmed Intl/ICU quirk: with hour12:false,
  // the "en-US" locale can format the midnight hour as "24" instead of
  // "0" (an artifact of deriving 24-hour formatting from 12-hour rules
  // internally) -- without this, isGoldMarketOpenAt incorrectly reported
  // the market as OPEN during the 00:00-00:59 ET hour specifically
  // (confirmed live: gold_market_open returned true at 00:55 ET on a
  // Sunday, hours before the real 6 PM ET reopen, tracked down via a
  // direct debug query against the tool's raw output). NOTE: this exact
  // quirk did NOT reproduce in a Node v22 sandbox test -- it may be
  // specific to Render's Node v20 runtime/ICU build, so this fix is
  // applied as a safe precaution (harmless either way) rather than a
  // fully confirmed root cause. See getGoldMarketDebugSnapshot() below,
  // added to directly verify the live server's actual computed values.
...
// TEMPORARY DEBUGGING AID -- exposes the raw ET time components and
// computed minute value the market-hours check actually used, so this
// can be verified directly against the live server's real behavior
// (which may differ from a local sandbox's Node version/ICU build)
// instead of guessing. Safe to remove once the market-hours bug is
// fully confirmed fixed and stable in production.
function getGoldMarketDebugSnapshot(checkDate) {
  ...
  const rawHourString = rawParts.find((p) => p.type === "hour").value;
  const rawWeekdayString = rawParts.find((p) => p.type === "weekday").value;
  const rawMinuteString = rawParts.find((p) => p.type === "minute").value;

  return {
    server_utc_iso: checkDate.toISOString(),
    raw_et_weekday_string: rawWeekdayString,
    raw_et_hour_string: rawHourString, // if this is literally "24", that confirms the ICU quirk on THIS server
    raw_et_minute_string: rawMinuteString,
    computed_is_open: isGoldMarketOpenAt(checkDate),
  };
}
...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  const response = {
    ...
    debug_market_calc: getGoldMarketDebugSnapshot(new Date()), // TEMPORARY -- see function definition for why
