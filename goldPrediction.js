...
export async function handleGoldPredictionCall(argsJson) {
  ...
  // Computed here deterministically (not left to GPT to parse/format the
  // raw ISO timestamp itself) -- same "compute it in code, hand GPT a
  // ready string" fix already applied to the direction/price consistency
  // bug. A prior version of the chatbot said "as of the last update"
  // with no actual date/time, which is a real, confirmed gap this closes.
