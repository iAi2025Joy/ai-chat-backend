...
export async function handleGoldPredictionCall(argsJson, userTimezone) {
  ...
  // A confirmed real, reproduced bug this fixes: the prose instruction
  // asking GPT to "mention plainly" a direction/price disagreement
  // (further down, in important_context_for_the_model) was NOT reliably
  // followed -- a real response stated "gold is likely to go down" and
  // a predicted_price_usd HIGHER than current_price_usd, with no
  // reconciling note at all, directly contradicting itself. This is the
  // exact same class of failure market_closed_statement was already
  // built to fix (see its own comment above): trusting GPT to compose
  // the right reconciling sentence from prose instructions alone,
  // buried in a long paragraph, is not reliable enough for something
  // this easy to get visibly wrong. Same fix: a pre-written, verbatim
  // sentence GPT is told to insert exactly as given, not compose itself.
  const predictionForNote = freshPrice.liveInferenceUsed ? freshPrice.liveInferenceDirection : (data.prediction ?? null);
  let directionPriceDisagreementNote = null;
  if (
    (predictionForNote === "up" || predictionForNote === "down") &&
    freshPrice.directionPriceAgreement === false &&
    freshPrice.priceDirectionVsCurrent
  ) {
    directionPriceDisagreementNote =
      `Note: the price-target model's specific number happens to come in ${freshPrice.priceDirectionVsCurrent} ` +
      `even though the direction call is ${predictionForNote} -- these are independent estimates from two ` +
      `separate models answering different questions (which way vs. what number), so this is normal, expected ` +
      `statistical noise, not a system error.`;
  }

  // DETERMINISTIC UNIFIED HEADLINE -- rebuilt per explicit request to
  // stop presenting the direction call and price target as two
  // separate, sometimes-disagreeing claims (a confirmed real
  // complaint: even with a reconciling note, showing both felt
  // confusing). Now leads with whichever signal actually has a real,
  // stated probability -- the direction model's own confidence (the
  // price-target model doesn't produce a genuine probability the same
  // way, only a significance flag) -- and treats the price purely as
  // an approximate supporting figure, not a second competing claim.
  // Confidence wording now actually scales with the real number
  // (weak/moderate/strong) instead of always sounding soft regardless
  // of how high it is -- a confirmed real complaint (78.7% was being
  // described as "only about 78.7%", undersold for a genuinely strong
  // reading).
  // A confirmed real, repeated complaint this fixes: even a single
  // unified sentence still felt contradictory whenever it mentioned a
  // direction word (up/down) alongside a price that technically moved
  // the other way -- per explicit request, direction is dropped from
  // this headline ENTIRELY now. Just the current price, the model's
  // specific predicted price, and (when available) a real market-
  // derived expected range -- a number and a range, no directional
  // claim to contradict anything.
  let unifiedPredictionHeadline = null;
  {
    const fmtUsd = (n) => typeof n === "number"
      ? `$${n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
      : null;
    const currentStr = fmtUsd(freshPrice.currentPriceUsd);
    const predictedStr = fmtUsd(freshPrice.predictedPriceUsd);
    if (currentStr && predictedStr) {
      // Defensive multi-name lookup -- the exact property names on this
      // object weren't directly confirmed from the Python source at
      // the time this was written, so several plausible names are
      // tried; if none match, the range clause is simply omitted
      // rather than risking a wrong/undefined value being shown.
      const ivRange = data.implied_volatility_expected_range || {};
      const rangeLow = ivRange.implied_price_range_low ?? ivRange.low ?? ivRange.range_low ?? null;
      const rangeHigh = ivRange.implied_price_range_high ?? ivRange.high ?? ivRange.range_high ?? null;
      const rangeLowStr = fmtUsd(rangeLow);
      const rangeHighStr = fmtUsd(rangeHigh);
      const rangeClause = (rangeLowStr && rangeHighStr)
        ? `, with a likely range of ${rangeLowStr} to ${rangeHighStr}`
        : "";
      unifiedPredictionHeadline =
        `Gold is currently at ${currentStr}. The model's specific price estimate for the next hourly update is ${predictedStr}${rangeClause}.`;
    }
  }
  ...
  const response = {
    ...
    direction_price_disagreement_note: directionPriceDisagreementNote,
    unified_prediction_headline: unifiedPredictionHeadline,
    ...
    important_context_for_the_model:
      "MANDATORY OPENING SENTENCE -- READ THIS FIRST, a real instruction that has now failed repeatedly despite multiple prose-only attempts, up to and including a directly self-contradictory live response stating 'increase' and 'leaning down' in the same reply -- per explicit request, direction (up/down) is no longer stated at all anywhere in your response, only a specific price and a range: unified_prediction_headline is a complete sentence ALREADY WRITTEN FOR YOU, assembled directly from the real numbers, giving a specific price and (when available) a likely range -- deliberately no directional claim at all, so there's nothing left to contradict. When gold_market_open is true, use it EXACTLY AS GIVEN, verbatim, word for word, as the very first sentence of your response, with nothing else before it -- do not paraphrase it, shorten it, reorder it, or compose your own version, and do NOT add your own direction/up/down statement anywhere else in the response either. This field is in English regardless of the conversation's language -- state it exactly as given even if the rest of your response is in a different language (this one sentence being English is a deliberate, known trade-off for guaranteed correctness). When gold_market_open is false, use market_closed_statement as the first sentence instead (see MARKET HOURS below) -- the two are mutually exclusive, never use both. Nothing else about the price/range belongs anywhere else in your response after this -- no second paragraph, no bullet, no 'Note:', no restating the numbers again in different words, and no separate direction/confidence statement (the 'prediction' and confidence fields still exist in the data for context, but do not lead with or restate them as a directional claim). " +
