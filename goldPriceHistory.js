...
export function getGoldPriceHistoryToolDefinition() {
  return {
    ...
    function: {
      ...
      description:
        "Get REAL recent historical gold price data (timestamp + USD/oz pairs) for when the user wants to SEE a chart, graph, line diagram, or trend of gold's price over roughly the last 24 hours (or a similar short window) -- this is NOT a prediction/forecast (use get_gold_prediction for that) and NOT just a single current price (use get_live_gold_price or search_web for that). Use this whenever the user asks to 'draw', 'plot', 'chart', 'graph', or 'show' gold's price trend, history, or movement over the last day / 24 hours / today. After calling this, you MUST call the render_chart function with type 'line' using this real data -- do not just describe the numbers in prose, and do not fabricate a chart without calling this function first.",
      parameters: {
        type: "object",
        properties: {
          hours: {
...
export async function handleGoldPriceHistoryCall(argsJson) {
  ...
  return JSON.stringify({
    ...
    important_context_for_the_model:
      "This is REAL historical price data, not a prediction. Present it to the user as a line chart by calling the render_chart function with type 'line', title (e.g. 'Gold Price - Last 24 Hours (USD/oz)'), labels built as short human-readable time labels derived from each point's time field, and data as the matching price numbers in the same order -- both arrays must be the same length as the points provided here. Do not fabricate, smooth, resample, interpolate, or add points beyond what was provided. Do NOT write a ```chart fenced code block yourself -- call render_chart directly, it renders the chart automatically. Always include a brief sentence of text before or after calling it (e.g. noting the actual time range covered and that this is historical, not predictive, data) -- never call render_chart with no surrounding text.",
  });
}
