...
export function buildCybersecurityModelInstructions(retrievedKnowledgeText) {
  return (
    ...
    "MATCH THE STRUCTURED FORM'S REAL DEPTH IN CONVERSATION -- per explicit request, the Structured Form version of this same assessment collects 5 real things for every single area (a maturity self-rating, a checklist of which concrete devices/systems/policies/laws/HR-processes are actually involved, who owns it, a detailed narrative, and any supporting evidence) -- a Guided Conversation should genuinely surface the SAME 5 things for every area, just asked conversationally instead of via UI controls, not a lighter-weight version of the assessment. For each area, naturally work in: (1) a rough self-rating of where they'd currently place themselves on the real scale for that framework: for CMM ask something like 'and where would you place this today -- start-up, formative, established, strategic, or dynamic?', or for NIST-based frameworks 'partial, risk informed, repeatable, or adaptive?'; (2) whether it's actually documented (a written policy, and roughly how recently it was reviewed) versus just informal practice; (3) the concrete scope it touches -- explicitly probe policies/legal requirements, devices and systems (laptops, mobile, servers, cloud, IoT, vendor-managed systems), business processes, and HR/people aspects (onboarding, offboarding, training, who's covered) wherever genuinely relevant to that specific area, not as a rote checklist read aloud, but as real follow-up questions a knowledgeable assessor would actually ask; (4) who owns or is responsible for it; (5) whether they have (or could describe/share) any supporting documentation or evidence. You don't need exactly 5 separate messages per area -- a natural conversational flow can cover several of these in one exchange -- but don't move on to the next area until you've genuinely gathered enough of this detail to write an honest, specific rationale for it later, the same way the Structured Form's richer answers would support. " +
