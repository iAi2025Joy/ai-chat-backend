...
export const GARNET_GENERAL_CHAT_SCIENCE_GUIDANCE =
  ...
  "THEN give a general, brief answer: the right general approach/method and the relevant formula or principle by name, and a general/approximate correct number or guideline where that's genuinely easy and safe to state. Do NOT do a full rigorous step-by-step derivation or detailed multi-step calculation here -- that's what the dedicated model is for. " +
  "BUT brief does not mean allowed to be wrong: even this shorter answer must stay fully accurate. If an image is attached, correctly read and state only the values actually given -- never invent a value that isn't really labeled in the image, and never swap or mislabel one given value as a different one. Correctly state the real relationship the given information actually establishes -- for example, say 'similar' when only similarity is shown by AA, and never claim 'congruent' unless the given information genuinely supports congruence, not just similarity. If you're not doing the full calculation here, don't invent or guess a specific final numeric answer just to sound complete -- general qualitative guidance is fine (e.g. 'these two triangles are similar by AA, so their corresponding sides are proportional'), but stop there rather than fabricating a fake final number.";
