...
export async function handleOilPredictionCall(userTimezone) {
  ...
  // Pre-written, ready-to-use rolling-accuracy sentence -- same pattern as
  // market_closed_statement/price_label below, and same fix as goldPrediction.js:
  // MUST be surfaced on every response, not just when the user asks about
  // accuracy specifically (confirmed real gap on the gold side, same code shape
  // here, so the same gap exists here too until fixed).
  ...
  if (trackedCount > 0 && rollingAccuracy !== null && rollingAccuracy !== undefined) {
    ...
    rollingAccuracyStatement =
      ...
      `prediction${trackedCount === 1 ? "" : "s"} (checked automatically, once per day, against real market prices).` +
    ...
    rollingAccuracyStatement =
      "Live tracked accuracy: no resolved predictions yet. The system automatically checks each daily prediction " +
      "against the real price the next day and is building up a real accuracy track record from scratch -- check back soon.";
  }

  // Pre-written disagreement sentence -- same rationale as gold: never let
  // 'prediction' and predicted_price_usd silently contradict each other in
  // the response without an explicit, unmissable callout.
  const disagreementStatement =
    data.direction_price_agreement === false
