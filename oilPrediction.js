...
import { fetchLiveOilPrice } from "./liveOilPrice.js";
import { runLiveOilInference } from "./oilLiveInference.js";

const OIL_PREDICTION_RAW_URL =
  "https://raw.githubusercontent.com/iAi2025Joy/gold-predictor/main/oil_prediction_latest.json";

// ------------------------------------------------------------------
// FRESH-PRICE-ON-REQUEST -- added to give oil the same live-inference
// capability gold already has, closing the real complexity gap between
// them (gold could answer with a genuinely fresh, on-demand price;
// oil always required a separate get_live_oil_price call for that).
// Same THREE-LAYER FALLBACK as gold, each strictly better than the next
// when available:
//  1. Full model inference: the live price genuinely flows through the
//     real trained model's real math -- see oilLiveInference.js. Not an
//     approximation.
//  2. Percentage scaling: if full inference isn't available (e.g. the
//     model artifacts file is missing, or not enough recent price
//     history), apply the model's already-decided predicted PERCENTAGE
//     change to the live price -- still live-price-aware, just not
//     re-derived from the model's actual feature-based logic.
//  3. Fully cached: if even the live price fetch itself fails, fall
//     back to the daily job's cached snapshot. A prediction should
//     never break just because one extra live call had a hiccup.
async function fetchFreshPriceData(cachedData) {
  let livePrice;
  try {
    const live = await fetchLiveOilPrice();
    // A synthetic (estimated/modeled, not a real observed trade) price
    // isn't a genuine live price -- don't let it silently flow through
    // real model inference as if it were one. Falls through to the
    // fully-cached layer below instead, same as an outright fetch failure.
    if (live.isSynthetic) {
      throw new Error("Live price is flagged synthetic -- not using it for live inference.");
    }
  ...
  if (cachedData.prediction === "up" || cachedData.prediction === "down") {
    const fullInference = await runLiveOilInference(livePrice, cachedData);
    if (fullInference) {
  ...
  let predictedPriceUsd = cachedPredicted;
...
export async function handleOilPredictionCall(userTimezone) {
  ...
  const freshPrice = await fetchFreshPriceData(data);
  ...
  const formattedLivePriceFetchedAt = livePriceFetchedAtDate && !isNaN(livePriceFetchedAtDate.getTime())
  ...
  const response = {
    ...
    live_inference_direction: freshPrice.liveInferenceDirection ?? null,
    live_inference_probability_up: freshPrice.liveInferenceProbabilityUp ?? null,
