...
// Exported (not just used internally) so other server-side modules --
// e.g. passwordReset.js -- can reuse this SAME initialized Admin SDK
// instance instead of duplicating the service-account setup, which
// would risk calling admin.initializeApp() twice and crashing.
...
// ------------------------------------------------------------------
// CHAT HISTORY VIEWING -- lets an admin see a user's saved chats. This
// is a genuinely privacy-sensitive capability (reading the actual
// content of someone's private conversations), so it uses the Admin
// SDK's Firestore access (which legitimately bypasses the normal
// client-side security rules that keep users' chats private from each
// other) -- gated behind the exact same verifyAdminRequest check as
// every other admin action, re-verified on every single request, not
// trusted from a prior check or from the frontend.
// ------------------------------------------------------------------

export async function handleListUserChats(req, res) {
  try {
    await verifyAdminRequest(req);
    getFirebaseAdmin();
    const { uid } = req.params;
...
export async function handleGetUserChatMessages(req, res) {
  try {
    await verifyAdminRequest(req);
    getFirebaseAdmin();
    ...
    const doc = await admin.firestore()
      .collection("users").doc(uid).collection("chats").doc(chatId).get();
