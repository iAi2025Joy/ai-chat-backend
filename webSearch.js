...
export function getWebSearchToolDefinition() {
  return {
    ...
    function: {
      name: "search_web",
      description:
        "Search the web for real, current information. Use this whenever the user asks something that needs up-to-date or real-time information you would not reliably know -- including, but not limited to, the CURRENT or LIVE price of gold or any other asset (as distinct from a gold PREDICTION/forecast, which uses the separate get_gold_prediction function instead). Also use for current events, recent news, or anything time-sensitive.",
      parameters: {
        type: "object",
        properties: {
          query: {
